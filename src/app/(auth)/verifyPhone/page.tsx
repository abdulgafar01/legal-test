...
export default function PhoneVerificationPage() {
  ...
  const verifyMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      ...
      const requiresProfileCompletion = data?.data?.requires_profile_completion || false;
      ...
      if (requiresProfileCompletion || isNewUser) {
        ...
        const userType = user?.user_type || localStorage.getItem('pendingUserType') || accountType;
    ...
    onError: (err: unknown) => {
      if (axios.isAxiosError(err)) {
        ...
        if (detail) {
          ...
          const msg = Array.isArray(phoneError) ? phoneError.join(' ') : phoneError;
          toast.error(msg);
        } else {
  ...
  const resendMutation = useMutation({
    ...
    onSuccess: (data) => {
      toast.success('OTP resent successfully to your phone.');
        ...
        <div className="bg-white rounded-2xl p-8 shadow-2xl">
          ...
          {debugMode && debugOtp && (
            <div className="mb-6 p-4 bg-yellow-50 border-2 border-yellow-400 rounded-lg">
              <div className="flex items-center gap-2 mb-2">
                <span className="text-yellow-700 font-semibold">🔧 DEBUG MODE</span>
              </div>
          ...
          <div className="mt-6 text-center">
            <Link href="/signup" className="text-sm text-gray-600 hover:underline">
              Wrong number? Go back to signup
            </Link>
          </div>
        </div>
      </div>
