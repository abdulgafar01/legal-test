...
export const WalletTab: React.FC = () => {
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      setHasMore(!!txData.next);
      setPage(nextPage);
    } catch (e) {} finally { setTxLoading(false); }
  };

  const handleWithdraw = async () => {
    if (!wallet || withdrawing || parseFloat(wallet.available_balance) <= 0) return;
    try {
      setWithdrawing(true);
  ...
  if (loading) {
    return <div className="flex items-center justify-center min-h-[300px]"><Loader2 className="h-6 w-6 animate-spin" /></div>;
  }
  if (error) {
      ...
      <Card className="bg-black text-white">
        ...
        <CardContent className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-gray-300 mt-1">Available for withdrawal</p>
          </div>
          <Button
            onClick={handleWithdraw}
            disabled={withdrawing || !wallet || parseFloat(wallet.available_balance) <= 0}
            className="bg-yellow-400 text-black hover:bg-yellow-300"
          >
            {withdrawing ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Request Withdraw'}
          </Button>
        </CardContent>
      </Card>
          ...
          {transactions.map(tx => {
            ...
            return (
              <li key={tx.id} className="p-4 flex items-start gap-4">
                <div className={`h-10 w-10 rounded-full flex items-center justify-center ${isIn ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                  <Icon className="h-5 w-5" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <p className="font-medium text-sm">
                      {tx.transaction_type === 'credit' && tx.counterparty_name ? `Consultation with ${tx.counterparty_name}` : tx.description}
                    </p>
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-3 text-xs text-gray-500">
                    ...
                    {tx.consultation_info && (
                      <span className="px-2 py-0.5 bg-gray-100 rounded-full">Consultation #{tx.consultation_info.id}</span>
                    )}
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${isIn ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{isIn ? 'Successful' : 'Processed'}</span>
                  </div>
                </div>
              </li>
            );
        ...
        {hasMore && (
          <div className="mt-4 flex justify-center">
            <Button variant="outline" onClick={loadMore} disabled={txLoading}>
              {txLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Load More'}
            </Button>
          </div>
        )}
      </div>
