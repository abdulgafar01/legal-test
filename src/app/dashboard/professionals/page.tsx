...
const Page = () => {
  ...
  const loadPractitioners = async (
    appliedFilters: FilterValues = filters,
    page: number = 1
  ) => {
    try {
      setLoading(true);
      ...
      if (
        appliedFilters.expertiseIds &&
        appliedFilters.expertiseIds.length > 0
      ) {
      ...
      if (appliedFilters.ratings) {
        ...
        if (!isNaN(minRating)) {
          practitioners = practitioners.filter(
            (p: any) => (p.average_rating || 0) >= minRating
          );
          console.log(
            `🔍 Filtered ${practitioners.length} practitioners with rating >= ${minRating}`
          );
        }
      }

      const count =
        response.pagination?.count || response.count || practitioners.length;
                ...
                {practitioners.map((practitioner) => (
                  <LawyerCard
                    ...
                    lawyer={{
                      ...
                      image:
                        practitioner.user_info.profile_image ||
                        "/placeholderImage.png",
                      rating: Math.round(practitioner.average_rating),
                      expertise: (() => {
                        const names =
                          practitioner.specializations?.map(
                            (s: any) => s?.name
                          ) || [];
                      ...
                      qualification:
                        practitioner.experience_level || "Legal Professional",
                      price: practitioner.hourly_rate,
                      location: practitioner.user_info.country,
                      status: practitioner.is_available_for_booking
                        ? "available"
                        : "booked",
                    }}
                  />
